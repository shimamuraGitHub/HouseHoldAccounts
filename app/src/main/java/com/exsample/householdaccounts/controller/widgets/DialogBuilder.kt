...
class DialogBuilder(context: Context) {
    ...
    fun buildMessage(message: Message) {
        builder.setTitle(message.title)
                .setMessage(message.message)
                .setPositiveButton("OK",null)
    }

    fun buildConfirm(message:ConfirmMessage, listener: (DialogInterface, Int) -> Unit) {
        builder.setMessage(message.message)
                .setPositiveButton(message.OK, listener)
                .setNegativeButton(message.CANCEL, null)
    }

    fun buildItems(message: ItemsMessage, listener: (DialogInterface, Int) -> Unit){
        builder.setTitle(message.title)
