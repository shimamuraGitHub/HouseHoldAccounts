...
class ConfigActivity : AbstractActivity() {
    ...
    override fun register() {
        val message = service.register(registerNameEdit, registerIsExpenditure)
        if (message.success) {
            resetRecordTypes()
    ...
    fun update(v: View) {
        ...
        buildDialog({ _, _ ->
            val from = service.findType(recordTypes,typeSpinner)
    ...
    fun erase(v: View) {
        ...
        buildDialog({ _, _ ->
            ...
            val message = service.erase(recordTypes,typeSpinner)
